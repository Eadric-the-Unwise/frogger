...
GameCharacter LOG_FROG;                                   // TOPHAT FROG
...
void update_frog_direction() {  // UPDATES THE FROG'S POSITION WHEN ON TURTLES AND LOGS
...
void init_level() {
    ...
    set_sprite_data(36, 4, log_frog_tiles);
...
void update_palette() {  // CORRECTLY UPDATES THE SPRITE COLOR PALETTES IF FLASHING IS ACTIVE. move_metasprite OVERWRITES PROPERTY FLAGS, SO THIS CORRECTS THOSE UPDATES
    UINT8 prop = 0;
...
void pause_game() {
    ...
    for (UINT8 *ptr = pause_text; *ptr != 0; ptr++) {  // when for loop hits a 0 (NOT A '0', WHICH IS ASCII), the loop ends
        set_vram_byte(vram_addr, *ptr + 96);
        vram_addr++;
    }
}
void unpause_game() {
    GAMESTATE = game;
    vram_addr = get_bkg_xy_addr(7, 15);
    for (UINT8 i = 5; i != 0; i--) {  // when for loop hits a 0 (NOT A '0', WHICH IS ASCII), the loop ends
...
void collide_tophat() {
    ...
    TOPHAT_L = (LOG_FROG.x + 2 & 255);      // NPC LEFT X
    TOPHAT_R = ((LOG_FROG.x + 14) & 255);   // NPC RIGHT X
    TOPHAT_T = LOG_FROG.y;                  // NPC TOP Y
    TOPHAT_B = LOG_FROG.y + 10;             // NPC BOTTOM Y

    // if (PLAYER_L <= TOPHAT_R && PLAYER_R >= TOPHAT_R || PLAYER_R >= TOPHAT_L && PLAYER_L <= TOPHAT_L) // X CROSSOVER COLLISION
    if (overlap(PLAYER.y, PLAYER_R, PLAYER.y + 10, PLAYER_L, LOG_FROG.y, TOPHAT_R, LOG_FROG.y + 10, TOPHAT_L) == 0x01U) {
        {
            LOG_FROG.spawn = FALSE;
            LOG_FROG.flash = FALSE;
            PLAYER.flash = TRUE;
